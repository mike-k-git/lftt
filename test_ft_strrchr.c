...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/16 16:56:09 by mkugan            #+#    #+#             */
/*   Updated: 2025/05/19 13:51:46 by mkugan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "tests.h"

typedef struct s_ft_strrchr_test {
	const char *name;
	const char *haystack;
	char c;
	int	cast;
...
void	test_ft_strrchr(int verbose)
{
	...
	for (size_t i = 0; i < num_tests; i++)
	{
		...
		if (tests[i].cast == 1)
			tests[i].c += 256;
		char	*result = ft_strrchr(tests[i].haystack, tests[i].c);
		char	*expected = strrchr(tests[i].haystack, tests[i].c);
		if (result == expected)
		{
			if (verbose)
			{
