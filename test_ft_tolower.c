...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/16 13:13:58 by mkugan            #+#    #+#             */
/*   Updated: 2025/05/19 13:52:40 by mkugan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "tests.h"

typedef struct s_ft_tolower_test {
	const char *name;
	int	c;
} t_ft_tolower_test;
...
void	test_ft_tolower(int verbose)
{
	...
	{
		total = total + 1;
		int	result = ft_tolower(tests[i].c);
		int	expected = tolower(tests[i].c);
		if ((result != 0) == (expected != 0))
		{
			if (verbose)
			{
				PRINT_PASS(tests[i].name);
				print_char_diff(expected, result);
			}
			else
				PRINT_PASS_SHORT();
			passed = passed + 1;
